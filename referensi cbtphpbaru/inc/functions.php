<?php
include_once __DIR__ . '/../koneksi/koneksi.php'; // Pastikan koneksi sudah ada
include_once __DIR__ . '/encrypt.php'; // berisi $method dan $rahasia

// Debug: Check if encryption variables are loaded
if (!isset($method) || empty($method)) {
    error_log("CRITICAL: Encryption method not loaded from encrypt.php");
    $method = "AES-256-CBC"; // Fallback
}

if (!isset($rahasia) || empty($rahasia)) {
    error_log("CRITICAL: Encryption key not loaded from encrypt.php");
    $rahasia = "@dmin12345"; // Fallback
}
    ...
    if ($user) {
        ...
        if ($role === 'admin') {
            ...
            if (!empty($user['force_logout'])) {
                ...
                mysqli_query($koneksi, "UPDATE siswa SET force_logout = FALSE, session_token = '' WHERE id_siswa = " . $user['id_siswa']);
                return false;
            }
...
function verify_admin_password($password_input, $stored_password) {
    // For experiment purposes, use plaintext comparison
