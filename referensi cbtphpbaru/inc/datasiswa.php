...
if (mysqli_stmt_execute($stmt)) {
    ...
    if ($user = mysqli_fetch_assoc($result)) {
        ...
        if (!empty($user['force_logout'])) {
            mysqli_query($koneksi, "UPDATE siswa SET session_token = '', force_logout = FALSE WHERE id_siswa = $id_siswa");
            session_unset();
            session_destroy();
...
$query_pengaturan = "SELECT * FROM pengaturan WHERE id = 1";
$pengaturan = mysqli_fetch_assoc(mysqli_query($koneksi, $query_pengaturan));
?>