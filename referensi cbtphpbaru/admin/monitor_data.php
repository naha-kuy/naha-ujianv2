...
$customSearch = isset($_GET['search']['value']) ? $_GET['search']['value'] : '';
$kodeSoalFilter = isset($_GET['kode_soal']) ? $_GET['kode_soal'] : '';
...
if (!empty($searchValue)) {
    $where .= " AND s.nama_siswa LIKE '%" . mysqli_real_escape_string($koneksi, $searchValue) . "%'";
} elseif (!empty($customSearch)) {
    $where .= " AND (s.nama_siswa LIKE '%" . mysqli_real_escape_string($koneksi, $customSearch) . "%' OR js.kode_soal LIKE '%" . mysqli_real_escape_string($koneksi, $customSearch) . "%')";
}

// Add kode soal filter
if (!empty($kodeSoalFilter)) {
    $where .= " AND js.kode_soal = '" . mysqli_real_escape_string($koneksi, $kodeSoalFilter) . "'";
}

// Add status filter
if (!empty($statusFilter)) {
    $where .= " AND js.status_ujian = '" . mysqli_real_escape_string($koneksi, $statusFilter) . "'";
}
