...
import ModalDuplicateSoal from "../components/modal/ModalDuplicateSoal";
...
export default function GuruBankSoal() {
  ...
  const [dupKode, setDupKode] = useState("");
    ...
    <div className="dash-layout">
                      ...
                      <tbody>
                        {paginated.map((s, i) => (
                          <tr key={s.id_soal}>
                            ...
                            <td style={{ fontFamily: "monospace", fontSize: 12, fontWeight: 700, color: "#5a3a00" }}>{s.token ? (s.token_required ? s.token : `${s.token} (ops)`) : "-"}</td>
                            <td className="td-actions">
                              {s.status === "Aktif" ? (
                                <>
                                  <button className="action-btn" title="Generate Token"
                            ...
                            <td className="td-actions">
                              ...
                              <button className="action-btn" title="Duplikasi"
                                ...
                                style={{ color: "#6f42c1", border: "1px solid #c9a8e8" }}>
                                <Icon name="save" size={14} />
                              </button>
                              <button className="action-btn" title="Butir Soal"
                                onClick={() => navigate(`/guru/butir-soal?kode_soal=${s.kode_soal}&nama=${encodeURIComponent(s.nama_soal)}`)}
      ...
      {showDup && (
        <ModalDuplicateSoal
          ...
          dupKode={dupKode}
          saving={saving}
          onClose={() => setShowDup(null)}
          onSave={handleDuplicate}
          onKodeChange={(v) => setDupKode(v)}
        />
      )}
    </div>
  );
